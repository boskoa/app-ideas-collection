...
const SimpleSliderC = ({ setA, setB }) => {
  ...
  const handleMouseDown = (event) => {
    diff.current = event.clientY - thumbRef.current.getBoundingClientRect().top;
  ...
  const handleMouseMove = (event) => {
    let newY =
      sliderRef.current.getBoundingClientRect().bottom -
      diff.current -
      event.clientY;
    ...
    thumbRef.current.style.bottom = getLeft(newPercentage);
